        ...
        @forelse($itemsForMember->where('stage', 2) as $find)
            ...
            <div class="mb-8 pb-8 border-b font-kanit">
                <div class="flex flex-col md:flex-row">
                    <img src="{{ asset($imagePath ?? 'path/to/default/image.jpg') }}" alt="{{ $find->type }}"
                        class="w-full h-auto md:w-32 md:h-32 object-cover mb-4 md:mb-0 md:mr-6 rounded-md">

                    <div>
                        <h2 class="text-xl font-semibold mb-2 font-kanit">{{ $find->item ?? 'ชื่อเรื่องที่ไม่ระบุ' }}</h2>
                        <p class="font-kanit">รายละเอียด</p>
                        <p class="text-gray-600 text-sm font-kanit">
                            <i class="fa-regular fa-clock"></i>
                            {{ \Carbon\Carbon::parse($find->created_at)->translatedFormat('d F Y, เวลา h:i A') }}
                            <span class="text-red-600">{{ $find->type }}</span>
                        </p>
