        ...
        @forelse($itemsForMember->where('stage', 1) as $found)
            ...
            <div class="mb-8 pb-8 border-b font-kanit">
                <div class="flex flex-col md:flex-row">
                    ...
                    <div>
                        <h2 class="text-xl font-semibold mb-2 font-kanit">{{ $found->item ?? 'ชื่อเรื่องที่ไม่ระบุ' }}</h2>
                        <p class="font-kanit">รายละเอียด</p>
                        <p class="text-gray-600 text-sm font-kanit">
                            <i class="fa-regular fa-clock"></i>
                            {{ \Carbon\Carbon::parse($found->created_at)->translatedFormat('d F Y, เวลา h:i A') }}
                            <span class="text-red-600">{{ $found->type }}</span>
                        </p>
