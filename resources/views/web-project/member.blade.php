...
@section('content')
    ...
    <header>
        <div class="container flex justify-center mt-8 max-w-full ">
            <button
                class="font-kanit text-white bg-orange-500 dark:bg-orange-600   rounded-full text-2xl px-24 py-3.5 text-center hover:bg-orange-700 mr-14">
                <a href="{{ route('popularItem') }}">ของที่หายบ่อย</a>
            </button>
            <button
                class="font-kanit text-white bg-orange-500 dark:bg-orange-600   rounded-full text-2xl px-24 py-3.5 text-center hover:bg-orange-700">
                <a href="#">ค้นหาของ</a>
            </button>
            <button
                class="font-kanit text-white bg-orange-500 dark:bg-orange-600   rounded-full text-2xl px-24 py-3.5 text-center hover:bg-orange-700 ml-14">
    ...
    <main>
        ...
        <div class="bg-slate-100 rounded-lg mt-5 flex flex-col items-center py-24 mx-auto w-3/4 shadow-2xl">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-6xl">
                @forelse($itemsForMember->where('stage', 3) as $find)
                    @php
                    $imageExtensions = ['jpg', 'png', 'jpeg', 'gif'];
                    $imagePath = null;
                    foreach ($imageExtensions as $ext) {
